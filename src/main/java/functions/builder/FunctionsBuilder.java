...
public class FunctionsBuilder {
	...
	private static StringBuilder generateClass(StringBuilder functionsBuilder, StringBuilder interfaceBuilder) {
		...
		BuilderLibrary.readBuilderLines(functionsBuilder, line -> {
			if (!line.isBlank()) {
				...
				System.out.println("dict: " + BuilderLibrary.extractParamTypesAndNames(line));
				String functionSignature = "public static " + BuilderLibrary.removeSemicolon(line) + " {\n";
				String functionBody = "\t";
				if (!BuilderLibrary.getFunctionTypes(line).get(0).equals("void")) // When return
					functionBody += "return ";
				functionBody += "MeosLibrary.meos." + BuilderLibrary.extractFunctionName(line) + "(" + BuilderLibrary.getListWithoutBrackets(BuilderLibrary.extractParamNames(line)) + ");\n}\n\n";
	...
	private static String changeFunctionType(String line) {
		if (!line.isBlank()) {
			...
			List<String> typesNotSupported = BuilderLibrary.getFunctionTypes(line).stream().filter(type -> !TYPES.containsValue(type)).toList(); // Retrieving unsupported types for the line
