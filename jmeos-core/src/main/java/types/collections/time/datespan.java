...
*/
    ...
    public LocalDateTime intersection(Object other) throws Exception {
        ...
        if (other instanceof LocalDate){
            ...
            datespanset ds = ((dateset) other).to_spanset(datespanset.class);
            Pointer resultPointer= GeneratedFunctions.intersection_spanset_span(ds.get_inner(), this.get_inner());
            int resultTimestamp= resultPointer.getInt(Integer.BYTES);
            result = timestampToLocalDateTime(resultTimestamp);
        }
        else if (other instanceof datespan){
            Pointer resultPointer = GeneratedFunctions.intersection_span_span(this._inner, ((datespan) other)._inner);
            int resultTimestamp= resultPointer.getInt(Integer.BYTES);
            result = timestampToLocalDateTime(resultTimestamp);
        }
        else if (other instanceof datespanset){
            Pointer resultPointer = GeneratedFunctions.intersection_spanset_span(((datespanset) other).get_inner(), this._inner);
            int resultTimestamp= resultPointer.getInt(Integer.BYTES);
            result = timestampToLocalDateTime(resultTimestamp);
        }
        else{
