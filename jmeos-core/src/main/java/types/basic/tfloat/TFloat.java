...
*/
	default float value_at_timestamp(LocalDateTime ts){
		Pointer ptr = GeneratedFunctions.tfloat_value_at_timestamptz(
				this.getNumberInner(), ConversionUtils.datetimeToTimestampTz(ts), true);
		if (ptr == null) throw new IllegalArgumentException("this has no value at the given timestamp");
		return (float) ptr.getDouble(0);
	}
/**
