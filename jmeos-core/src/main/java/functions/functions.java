...
public class functions {
	...
	// ───────────────────────────────────────────────────────────────────────
	// MeosLibrary split into 4 public static sub-interfaces to avoid the
	// JVM MethodTooLargeException (64 KB limit) on ARM64/Windows/macOS.
	// Each JNR-FFI proxy handles ≤ 430 methods. The public static wrappers
	// below call the correct proxy; MeosLibrary.meos still works via a
	// lightweight reflection delegate (zero per-method boilerplate).
	// ───────────────────────────────────────────────────────────────────────

	public static interface MeosLibraryPartA {
	...
	public static interface MeosLibraryPartB {
	...
	public static interface MeosLibraryPartC {
	...
	public static interface MeosLibraryPartD {
	...
	/**
	 * @deprecated Loading via JNR-FFI causes MethodTooLargeException on ARM64.
	 *   Use the public static wrappers below. {@code MeosLibrary.meos} still
	 *   works; it is now a reflection delegate routing to PartA–D proxies.
	 */
	@Deprecated
	public interface MeosLibrary
			extends MeosLibraryPartA, MeosLibraryPartB, MeosLibraryPartC, MeosLibraryPartD {
		String libraryPath = _LIB;
		@SuppressWarnings({"unchecked","deprecation"})
		MeosLibrary INSTANCE = (MeosLibrary) java.lang.reflect.Proxy.newProxyInstance(
				MeosLibrary.class.getClassLoader(),
				new Class<?>[]{ MeosLibrary.class },
				(proxy, method, args) -> {
					Object target = _dispatch.get(method.getName());
					if (target == null)
	...
	public static Pointer tfloat_values(Pointer temp, Pointer count) {
		var _result = _meos_c.tfloat_values(temp, count);
		MeosErrorHandler.checkError();
		return _result;
	}
	...
	public static Pointer tint_value_time_boxes(Pointer temp, int vsize, Pointer duration, int vorigin, OffsetDateTime torigin, Pointer count) {
		var torigin_new = torigin.toEpochSecond();
		var _result = _meos_d.tint_value_time_boxes(temp, vsize, duration, vorigin, torigin_new, count);
		MeosErrorHandler.checkError();
		return _result;
	}
	...
	public static Pointer tint_value_time_split(Pointer temp, long size, Pointer duration, int vorigin, OffsetDateTime torigin, Pointer value_bins, Pointer time_bins, Pointer count) {
		var torigin_new = torigin.toEpochSecond();
		var _result = _meos_d.tint_value_time_split(temp, size, duration, vorigin, torigin_new, value_bins, time_bins, count);
		MeosErrorHandler.checkError();
		return _result;
	}
